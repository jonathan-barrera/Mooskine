...
extension NoteDetailsViewController {
    ...
    @IBAction func boldTapped(sender: Any) {
        ...
        let attributeValue = UIFont(name: "OpenSans-Bold", size: 22)!
        var attributesDict: [NSAttributedString.Key : Any] = [:]
        
        newText.enumerateAttributes(in: textView.selectedRange, options: .longestEffectiveRangeNotRequired) {
            (attributes, range, stop) in
            attributesDict = attributes
        }
        
        if attributesDict[.font] as! UIFont != attributeValue {
            // Should put font info in a struct/enum
            newText.addAttribute(.font, value: attributeValue, range: textView.selectedRange)
        } else {
            let defaultAttribute = UIFont.systemFont(ofSize: UIFont.labelFontSize)
            newText.addAttribute(.font, value: defaultAttribute, range: textView.selectedRange)
        }
    ...
    @IBAction func redTapped(sender: Any) {
        ...
        if attributesDict[.underlineStyle] != nil && attributesDict[.underlineStyle] as! Int == 1 {
            let defaultAttributes:[NSAttributedStringKey: Any] = [
                .foregroundColor: UIColor.black,
                .underlineStyle: 0
            ]
            newText.addAttributes(defaultAttributes, range: textView.selectedRange)
        } else {
    ...
    @IBAction func cowTapped(sender: Any) {
        ...
        backgroundContext.perform {
            ...
            let cowText = Pathifier.makeMutableAttributedString(for: selectedText, withFont: UIFont(name: "AvenirNext-Heavy", size: 55)!, withPatternImage:  #imageLiteral(resourceName: "toolbar-cow"))
